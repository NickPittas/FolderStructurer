...
# =========== Tab 3: AddToExistingProjectTab ===========

class AddToExistingProjectTab(QWidget):
    """
    Third tab:
      - "Create Nuke Script under comp" toggles live preview
      - same resolution presets as Tab 1
      - QTreeWidget with checkable items
      - SequenceListContainer
      - real-time preview
      - "Add Folders & Shots" => create subfolders, optional .nk script if parent has 'comp'
    """
    def __init__(self, main_window):
        super().__init__()
        self.main_window = main_window
        ...
        lay.addWidget(self.chk_create_nk_under_comp)
        
        # 3) Res / FPS / Proxy / ACES (same dict as Tab 1)
        row_res= QHBoxLayout()
        lbl_rp= QLabel("Resolution Preset:")
        self.combo_res_presets= QComboBox()
        self.combo_res_presets.addItem("Custom")
        
        # copy same dictionary from Tab 1
        self.res_presets= {
        ...
        row_pa.addWidget(self.chk_proxy)
        row_pa.addWidget(self.chk_aces)
        lay.addLayout(row_pa)
        ...
        # so that item check changes => preview
        self.tree.itemChanged.connect(self.update_preview_tab3)
        
        # 5) SequenceListContainer
        self.seq_container= SequenceListContainer(self.main_window)
        # add 1 sequence by default
        ...
        lay.addWidget(scroll_seq)
        
        # 6) Preview
        lbl_prev= QLabel("Preview of changes:")
        self.text_preview= QTextEdit()
        self.text_preview.setReadOnly(True)
        lay.addWidget(lbl_prev)
        lay.addWidget(self.text_preview)
        ...
        self.combo_res_presets.currentIndexChanged.connect(self.update_preview_tab3)
        ...
        self.chk_create_nk_under_comp.stateChanged.connect(self.update_preview_tab3)  # <--- so toggling also updates preview
    
    def on_browse_project(self):
        fol= QFileDialog.getExistingDirectory(self, "Select Existing Project Folder")
        if fol:
            self.edit_project_folder.setText(fol)
            self.populate_tree(fol)
    ...
    def folder_or_parents_has_comp(self, folder_path, project_root):
        ...
        while len(folder_path) >= len(project_root):
            ...
            if "comp" in base:  # or base.startswith("comp")
                return True
    ...
    def on_execute(self):
        ...
        # gather resolution/fps/proxy/aces
        sel_rp= self.combo_res_presets.currentText()
        w_val= self.spin_width.value()
        h_val= self.spin_height.value()
        res_label= f"Custom_{w_val}x{h_val}"
        ...
        for citem in checked_items:
            ...
            # see if or any parent folder has "comp" 
            ...
            for sd in seq_data:
                ...
                if seqnm:
                    ...
                    for sh in shots:
                        ...
                        # if comp => create project/render + .nk
                        if is_comp_folder:
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("3-Tab Script with Full Res Presets & comp-based .nk creation in Tab3")
        self.resize(900,600)
        ...
        # 3) Tab 3 => "Add to Existing Project"
        self.tab3 = AddToExistingProjectTab(self)
        self.tab_widget.addTab(self.tab3, "Add to Existing Project")
    
    def build_tab1_ui(self):
        # Standard approach: Hardcoded vs Template, big dictionary of res presets, etc.
        # The same code from your snippet
        lay = QVBoxLayout()
        lay.setSpacing(5)
        lay.setContentsMargins(10,10,10,10)
        ...
        lay.addLayout(row_template)
        
        row_res= QHBoxLayout()
        lbl_pres= QLabel("Resolution Preset:")
        self.combo_res_presets= QComboBox()
        self.combo_res_presets.addItem("Custom")
        ...
        lay.addLayout(row_proxy_aces)
        
        # Sequences
        self.sequence_container= SequenceListContainer(self)
        self.scroll_seq= QScrollArea()
        self.scroll_seq.setWidgetResizable(True)
        self.scroll_seq.setWidget(self.sequence_container)
        lay.addWidget(self.scroll_seq)
    ...
    # Then place your Hardcoded vs Template creation logic, 
    # build_preview_hardcoded, create_hardcoded_structure, replicate_template_structure, etc.
    # plus the nuke script builder. 
    # Full code is the same approach from your prior snippet.
